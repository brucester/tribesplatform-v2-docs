...
export default function OnboardingPage() {
  ...
  // Offers + requests
  const [offers, setOffers] = useState<{ category: string; title: string; description: string }[]>([
    { category: 'skills', title: '', description: '' }
  ])
  const [requests, setRequests] = useState<{ category: string; title: string; description: string }[]>([
    { category: 'community', title: '', description: '' }
  ])
  ...
  const checkUsername = async (val: string) => {
    if (!val || val.length < 3) { setUsernameError('Username must be at least 3 characters'); return }
    ...
    if (data) setUsernameError('This username is taken')
  ...
  const addOffer = () => setOffers(o => [...o, { category: 'skills', title: '', description: '' }])
  const addRequest = () => setRequests(r => [...r, { category: 'community', title: '', description: '' }])
      ...
      <div style={{ maxWidth: 560, margin: '0 auto' }}>
        ...
        {step === 1 && (
          <div>
            ...
            <p style={{ color: 'var(--ink-3)', fontSize: 14, marginBottom: 32 }}>
              This is your public presence in the Regen Tribe network.
            </p>
            ...
            <Field label="Username" required hint={usernameError || 'Your public URL: regentribe.com/u/username'} error={!!usernameError}>
              <input
                value={username}
            ...
            <Field label="Bio" hint="What brings you to regenerative living?">
              <textarea value={bio} onChange={e => setBio(e.target.value)} rows={4}
                placeholder="A few sentences about who you are, what you've done, and where you're heading…" />
            </Field>

            <button
              onClick={saveProfile} disabled={loading || !displayName || !username || !!usernameError}
              style={{
        ...
        {step === 2 && (
          <div>
            ...
            <p style={{ color: 'var(--ink-3)', fontSize: 14, marginBottom: 32 }}>
              These are the heart of the matching system. Add as many as you like — you can always edit later.
            </p>
            ...
            <div style={{ marginBottom: 32 }}>
              <div style={{
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: 15 }}>🌱 What you offer</div>
                  <div style={{ fontSize: 12.5, color: 'var(--ink-3)' }}>Skills, resources, land, funding, expertise…</div>
                </div>
              ...
              {offers.map((o, i) => (
                <div key={i} style={{
                  ...
                  <input value={o.description} placeholder="A bit more detail (optional)"
            ...
            <div style={{ marginBottom: 40 }}>
              <div style={{
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: 15 }}>🔍 What you seek</div>
                  <div style={{ fontSize: 12.5, color: 'var(--ink-3)' }}>Community, skills, co-founders, resources…</div>
                </div>
                <button onClick={addRequest} style={{
                  fontSize: 12.5, color: 'var(--warn)', background: 'var(--warn-soft)',
                  border: 'none', padding: '5px 12px', borderRadius: 20, fontWeight: 600,
                }}>+ Add request</button>
              </div>
              {requests.map((r, i) => (
                <div key={i} style={{
                  ...
                  <input value={r.description} placeholder="A bit more detail (optional)"
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={() => setStep(1)} style={{
                flex: '0 0 auto', padding: '12px 20px', borderRadius: 'var(--radius)',
            ...
            <p style={{ textAlign: 'center', fontSize: 12.5, color: 'var(--ink-3)', marginTop: 12 }}>
              You can skip this and add offers/requests from your profile page later.
            </p>
          </div>
        )}
      </div>
...
function Field({ label, children, hint, required, error }: {
  ...
  hint?: string; required?: boolean; error?: boolean;
}) {
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      {children}
      {hint && (
        <div style={{ fontSize: 12, marginTop: 5, color: error ? '#c0392b' : 'var(--ink-3)' }}>{hint}</div>
      )}
    </div>
  )
}
